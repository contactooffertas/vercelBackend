  ...
  try {
    ...
    try {
      await transporter.sendMail({
        ...
        to:      email,
        subject: '🔐 Código de verificación — Offerton',
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from:    '"Offerton" <contacto.offertas@gmail.com>',
        to:      email,
        subject: '🔄 Nuevo código de verificación — Offerton',
        text:    `Tu nuevo código es: ${newCode}. Válido por 10 minutos.`,
        html:    resendEmailHTML(newCode, user.name),
      });
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from:    '"Offerton" <contacto.offertas@gmail.com>',
        to:      email,
        subject: '🔑 Recuperar contraseña — Offerton',
        text:    `Tu código para restablecer la contraseña es: ${code}. Válido por 15 minutos.`,
        html:    forgotPasswordHTML(code, user.name),
      });
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from:    '"Offerton" <contacto.offertas@gmail.com>',
        to:      email,
        subject: '✅ Contraseña actualizada — Offerton',
        text:    'Tu contraseña fue actualizada correctamente.',
        html:    passwordChangedHTML(user.name),
      });
